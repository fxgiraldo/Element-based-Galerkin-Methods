...
for e=1:nelem
   ...
   DL=h_L*dqdx_diss; %Dirichlet BCs
   DR=h_R*dqdx_diss;
   NL=n_L*dhdx_L*(q_L-q_star); %Neumann BCs
   NR=n_R*dhdx_R*(q_R-q_star);
...
%    rhs(IR)=rhs(IR) + n_R*h_R*dqdx_mean - h_R*dqdx_diss + n_R*dhdx_R*(q_R-q_star);
   rhs(IL)=rhs(IL) + n_L*h_L*dqdx_mean - DL + NL; 
   rhs(IR)=rhs(IR) + n_R*h_R*dqdx_mean - DR + NR;
end