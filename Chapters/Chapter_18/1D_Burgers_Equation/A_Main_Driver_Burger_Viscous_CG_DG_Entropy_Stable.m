%---------------------------------------------------------------------%
%This code solves the 1D Viscous Burgers Equation using either CG or 
...
%RK.
%Written by F.X. Giraldo on 12/2/2018
...
%Extract frames to plot
nframes=0;
pointer_plot_frames=zeros(nplot_frames);
for i=1:nplot_frames
    jbeg=1;
    for j=1:iframe
        if abs(time_movie(j)-time_plot_frames(i)) < 1e-5
            nframes=nframes + 1;
            pointer_plot_frames(nframes)=j;
            jbeg=j+1;
            break
        end
    end
...
%             elseif (i==3)
%                 plot_handle=plot(x,y,'k:','LineWidth',2);
%             elseif (i==4)
%                 plot_handle=plot(x,y,'m-.','LineWidth',2);
%             end
%             hold on;
%         end
%         axis([xmin xmax hmin hmax]);
