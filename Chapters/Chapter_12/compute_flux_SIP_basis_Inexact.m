...
function [psi_x, psi_y] = compute_flux_SIP_basis_Inexact(psi,dpsi,ngl,nq,...
                           ksi_x,ksi_y,eta_x,eta_y,iloc,e,i,l,imap)
...
for i=1:ngl
  ...
  h_k=psi(jl,ll);
  h_jk=h_j*h_k;
  psi_e=psi_e + dpsi(il,kl)*psi(jl,ll);
  psi_n=psi_n + psi(il,kl)*dpsi(jl,ll);
end %i 
psi_x=psi_e*ksi_x(e,kl,ll) + psi_n*eta_x(e,kl,ll);
psi_y=psi_e*ksi_y(e,kl,ll) + psi_n*eta_y(e,kl,ll);
...
%   h_k=psi(jl,ll);
%   h_jk=h_j*h_k;
%   psi_e=dpsi(il,kl)*psi(jl,ll);
%   psi_n=psi(il,kl)*dpsi(jl,ll);
%   psi_x=psi_e*ksi_x(e,kl,ll) + psi_n*eta_x(e,kl,ll);
%   psi_y=psi_e*ksi_y(e,kl,ll) + psi_n*eta_y(e,kl,ll);
% end %i 
