...
%----------------------------------------------------------------------%
function rhs = compute_flux(rhs,q,psideh,nx,ny,jac_side,jac,psi,dpsi,...
       nside,ngl,nq,imapl,imapr,intma,ksi_x,ksi_y,eta_x,eta_y,mu_constant)
...
for is=1:nside
    ...
    for l=1:nq
        ...
        [ql_k, ql_x, ql_y] = compute_SIP_flux_terms(ql,psi,dpsi,ngl,nq,...
                             ksi_x,ksi_y,eta_x,eta_y,ilocl,el,l);                  
        
        %Interpolate Right-State onto Quadrature Points
        if (er > 0)
            [qr_k, qr_x, qr_y] = compute_SIP_flux_terms(qr,psi,dpsi,ngl,nq,...
                                 ksi_x,ksi_y,eta_x,eta_y,ilocr,er,l);
        elseif (er == -4)
        ...
        for i=1:ngl
          ...
          %Left States
          h_i=psi(i,l);          
          [psil_x, psil_y] = compute_SIP_flux_basis(psi,dpsi,ngl,nq,...
                           ksi_x,ksi_y,eta_x,eta_y,ilocl,el,i,l);
          ...
          %--------------Right Side------------------%
          if (er > 0)
             [psir_x, psir_y] = compute_SIP_flux_basis(psi,dpsi,ngl,nq,...
                           ksi_x,ksi_y,eta_x,eta_y,ilocr,er,i,l);
