...
%---------------------------------------------------------------------%
function [qp,element_order] = change_polynomial_order(qp,VDM_Matrix,VDM_Inv_Matrix,nelem,element_order,p_tol,p_tol2,nop_min,nop_max)
eps=1e-15;
...
for e=1:nelem
   ...
   if (check <= p_tol || denominator <= p_tol2) %Coarsen
       if (i>=nop_min+2)
...
%            disp(['(1) Coarsen: e= ',num2str(e),' p= ',num2str(i),' num = ',num2str(numerator),' den= ',num2str(denominator),' check= ',num2str(check)]);
   ...
   elseif (check > p_tol && denominator > p_tol2) %Refine
       if (i<nop_max+1)
