...
%----------------------------------------------------------------------%
function [iside,jeside,jesideh] = create_side(intma,bsido,npoin,nelem,nboun,nside,ngl,ae)

%global arrays - four times bigger than without refinement
iside = zeros(nside,4);
jeside= zeros(nelem,4);
jesideh= zeros(nelem,4);

%local arrays
lwher = zeros(npoin,1);
lhowm = zeros(npoin,1);
icone = zeros(ae*npoin,1);
inode = zeros(4,1);
jnode = zeros(4,1);
