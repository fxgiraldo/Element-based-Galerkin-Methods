%---------------------------------------------------------------------%
%       BEST LGR CODE I have as of 4/12/2024
%---------------------------------------------------------------------%
...
%Written by F.X. Giraldo on 04/2024
...
write_reference_solution=0;
write_reference_solution_filename='1D_New_SWE_NonLinear_Exact_t=0p5.mat';
%----------------------------------Plotting Variables----------------------%
...
experiment_method=2; %0=LGL with NFBC
                     ...
                     %3=LGR NRBC with Sponge
                     %4=LGL NRBC: similar to LGR in that the sponge zone is scaled by 20x 
                     ...
                     %9=same as experiment 3: LGR NRBC but without a Sponge (LGR element scaled to 8x)
                     %10=same as experiment 9: LGR NRBC without Sponge but domain scaled to 20x (instead of 8)
                     %11=same as experiment 3: LGR NRBC with Sponge but domain scaled to 8x and minimal sponge amplitude = 1e-4
                     %12=same as experiment 10: LGR NRBC with domain scaled to 20x but with minimal sponge amplitude = 1e-4
%------------------Allowed to CHANGE-------------------%
...
if (delta_nl == 0) 
    ...
    Courant_max=0.25; 
    time_final=2.0; 
    reference_solution_filename='1D_New_SWE_Linear_Exact_t=2.mat';    
elseif (delta_nl == 1) 
    visc=2e-2;
    Courant_max=0.125;
    time_final=0.5; 
    reference_solution_filename='1D_New_SWE_NonLinear_Exact_t=0p5.mat';
end
...
%----LGL Reference Solution (lsponge=1 but sponge_amp=0)
elseif (experiment_method==1)
    ...
    LGR_scale=1; %1=scale and 0=do not scale
    LGR_scale_factor=1.25; %factor to scale from xgr_scale
    LGR_basis='LGL'; %LGL or LGR
    LGR_BC=0; %1=homogeneous Dirichlet BC, 0=turn it off
    lsponge=1;
    sponge_amp=0; 
...
elseif (experiment_method==4)
    ...
    LGR_scale=1; %1=scale and 0=do not scale
    LGR_scale_factor=20;%nop_LGR; %factor to scale from xgr_scale
    LGR_basis='LGL'; %LGL or LGR
    LGR_BC=0; %1=homogeneous Dirichlet BC, 0=turn it off
    lsponge=1;
    xmax_plot_type=1; %1=xmax_LGR, 2=xmax_LGL
...
elseif (experiment_method==9)
    ...
    LGR_artificial_layer=1; %=1 increases size of domain; =0 keeps the same max/mins of domain
    LGR_scale=1; %1=scale and 0=do not scale
    LGR_scale_factor=8; %8; %factor to scale from xgr_scale
    LGR_basis='LGR'; %LGL or LGR
    LGR_BC=1; %1=homogeneous Dirichlet BC, 0=turn it off
    lsponge=1;
    sponge_amp=0;
...
elseif (experiment_method==10)
    ...
    LGR_artificial_layer=1; %=1 increases size of domain; =0 keeps the same max/mins of domain
    LGR_scale=1; %1=scale and 0=do not scale
    LGR_scale_factor=20; %8; %factor to scale from xgr_scale
    LGR_basis='LGR'; %LGL or LGR
    LGR_BC=1; %1=homogeneous Dirichlet BC, 0=turn it off
    lsponge=1;
    sponge_amp=0;
...
%----LGR NRBC 8x LGR element with minimal sponge: 
elseif (experiment_method==11)
    nelem_LGL=32; %Number of Elements
    nelem_LGR=1;
    nop_LGL=4;    %Polynomial Order
    nop_LGR=32;
...
%----LGR NRBC 20x LGR element with minimal sponge: 
elseif (experiment_method==12)
    nelem_LGL=32; %Number of Elements
    nelem_LGR=1;
    nop_LGL=4;    %Polynomial Order
    nop_LGR=32;
    ...
    LGR_artificial_layer=1; %=1 increases size of domain; =0 keeps the same max/mins of domain
    LGR_scale=1; %1=scale and 0=do not scale
    LGR_scale_factor=20; %factor to scale from xgr_scale
    LGR_basis='LGR'; %LGL or LGR
    LGR_BC=1; %1=homogeneous Dirichlet BC, 0=turn it off
    lsponge=1;
    sponge_amp=1e-2;
...
if (write_reference_solution==1)
    q_ref=qp;
    coord_ref=coord_cg;
    save(write_reference_solution_filename,'q_ref','coord_ref');
end
%----------------------------------Write Solution to File
...
if (plot_figures == 1)
    ...
    if (plot_reference_solution==1)
        load(reference_solution_filename,'q_ref','coord_ref');
        plot(coord_ref,q_ref(:,1),'k:','LineWidth',2);
        hmax_ref=max(q_ref(:,1));
        hmin_ref=min(q_ref(:,1));
        hmax=max(hmax,hmax_ref);
        hmin=min(hmin,hmin_ref);
    end
    axis([xmin xmax hmin hmax]);
    ...
    hold on;
    if (plot_reference_solution==1)
        plot(coord_ref,q_ref(:,2),'k:','LineWidth',2);
        hmax_ref=max(q_ref(:,2));
        hmin_ref=min(q_ref(:,2));
        hmax=max(hmax,hmax_ref);
        hmin=min(hmin,hmin_ref);
    end
    axis([xmin xmax hmin hmax]);
