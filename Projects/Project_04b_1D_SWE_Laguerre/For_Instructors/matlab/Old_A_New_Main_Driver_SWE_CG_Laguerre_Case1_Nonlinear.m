...
Courant_max=0.125; %This is what controls the time-step
...
plot_mass=0;
plot_bathymetry=0;
plot_partial_domain=1; %2=only plot right-half of domain, 1=only plot part of domain, 0=plot entire domain
plot_reference_solution=1;
%----------------------------------Plotting Variables----------------------%

%------------------Allowed to CHANGE-------------------%
%Defines Test Case
time_final=0.5; %final time in revolutions: use =1 for NFBC and =10 for NRBC
...
experiment_method=10; %0=LGL with NFBC
                     ...
                     %9=same as experiment 3: LGR NRBC but without a Sponge
                     %(LGR element scaled to 8x)
                     %10=same as experiment 9: LGR NRBC without Sponge but
                     %domain scaled to 20x (instead of 8)
%------------------Allowed to CHANGE-------------------%
...
if (plot_figures == 1)
    ...
    if (plot_reference_solution==1)
        load('1D_SWE_NL_Exact_t=0p5.mat','q_ref','coord_ref');
