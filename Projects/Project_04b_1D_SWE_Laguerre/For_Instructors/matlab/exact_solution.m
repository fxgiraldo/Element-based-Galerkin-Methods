...
%---------------------------------------------------------------------%
function [qe,qb,gravity] = exact_solution(intma,coord,npoin,nelem,ngl,time,icase)
...
for e=1:nelem
   for i=1:ngl(e)
      ...
      if (icase == 0) %Gaussian IC with flat bottom: Advection
       gravity=1;
       hmean=0;
       amp=1.0;
       strength=16;
       ...
       qe(ip,2)=2.0;   
       qb(ip)=hmean;
      elseif (icase == 1) %Gaussian IC with flat bottom
       gravity=10;
       hmean=0.1;
       hb=0;
       amp=1.0;
       ...
       qe(ip,2)=0;   
       qb(ip)=hmean;
      elseif (icase == 2) %Gaussian IC with flat bottom
       gravity=10;
       hmean=1.0;
       hb=0;
       amp=0.1;
       ...
       qe(ip,2)=0;   
       qb(ip)=hmean;
      elseif (icase == 3) %Gaussian IC with Linear Bottom
       gravity=10;
       hmean=0.2;
       hb=hmean - 0.1*(1 - (x-xmin)/(xmax-xmin));
       % hmean=0.5;
       % hb=hmean - 0.25*(1 - (x-xmin)/(xmax-xmin));
       hh=amp*exp( -strength*r^2 );
       qe(ip,1)=hh;
       qe(ip,2)=0;   
       qb(ip)=hb;
      elseif (icase == 4) %Gaussian IC with Parabolic Bottom
       gravity=10;
       hmean=0.2;
       hh=amp*exp( -strength*r^2 );
       ...
       qe(ip,2)=0;   
       qb(ip)=hb;
      elseif (icase == 5) %Standing Wave with Analytic Solution
       c=2;
       gravity=1;
       hmean=1.0;
       ...
       elseif (icase == 6) %Dam-Break Problem
          ...
          qe(ip,2)=uu;   
          qb(ip)=hb;
       elseif (icase == 7) %Dam-Break Problem
          gravity=9.81;
          hb=0;
