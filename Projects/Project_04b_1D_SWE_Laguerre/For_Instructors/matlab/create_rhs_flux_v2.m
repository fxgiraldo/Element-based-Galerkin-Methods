...
%---------------------------------------------------------------------%
function rhs = create_rhs_flux(rhs,qp,qb,intma,nelem,ngl,diss,gravity,delta_nl,lsponge,nrbc_right)
...
%if (lsponge==0)
    ...
    hs_r=hs_l;
    if (nrbc_right==1)
        U_r =-U_l; %hard wall boundary
    elseif (nrbc_right==2)
        U_r =U_l; %extrapolation/outflow boundary
    end
    hb_r=hb_l;
    u_r =U_r/(h_r);   
    %Store States
