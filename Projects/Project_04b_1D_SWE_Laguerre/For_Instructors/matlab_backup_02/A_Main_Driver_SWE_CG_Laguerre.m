...
%Also, it can solve advection by using icase==-1. All other cases uses SWE.
...
%sponge variables
sponge_amp=1.0; %factor that multiplies sponge. >1 is stronger
sponge_exponent=4.0;
sponge_shape=1; %1=sin, 2=tanh
...
%----------------------------------Plotting Variables----------------------%
%Plotting variables
nplots=50; %plotting variable - How many Frames
iprint=100; %how often to print time-step info
plot_movie=1;
plot_figures=1;
plot_figures_title=1;
plot_elements=1;
plot_qmax=0;
plot_visc=0;
plot_mass=0;
...
%------------------Allowed to CHANGE-------------------%
%Defines Test Case
time_final=3.5; %final time in revolutions: use =1 for NFBC and =10 for NRBC
delta_nl=1; %=0 linear and =1 nonlinear
icase=0; %=-1 is a Gaussian exp(-16r^2) Advection as used in many other sample problems in Giraldo 2020 book
         %=0 is a Gaussian exp(-16r^2) as used in many other sample problems in Giraldo 2020 book
experiment_method=5; %1=LGL with NFBC
                     %2=LGL with NRBC Standard: portion of the domain is the sponge
                     %3=LGR NRBC 
                     ...
                     %7=LGL NRBC: with outflow/extrapolation BC and No Sponge
...
if (experiment_method==1)
    nelem_LGL=31; %Number of Elements
    nelem_LGR=1;
...
elseif (experiment_method==2)
    nelem_LGL=24; %Number of Elements
    nelem_LGR=8;
    nop_LGL=4;    %Polynomial Order
    nop_LGR=4;
    nelem=nelem_LGL+nelem_LGR;
...
elseif (experiment_method==3)
    ...
    LGR_scale=0; %1=scale and 0=do not scale
    LGR_scale_factor=nop_LGR; %factor to scale from xgr_scale
...
elseif (experiment_method==7)
    ...
    LGR_artificial_layer=1; %=1 increases size of element =0 keeps the same max/mins of domain
    LGR_scale=1; %1=scale and 0=do not scale
    LGR_scale_factor=1e0; %factor to scale from xgr_scale
    LGR_basis='LGL'; %LGL or LGR
    LGR_BC=0; %1=homogeneous Dirichlet BC, 0=turn it off
    lsponge=1;
    sponge_amp=0;
...
%Create Grid
[coord,coord_cg,intma,jac,wnq,npoin] = create_grid(ngl,ngl_LGR,nelem,nelem_LGL,xgl,xgr,wgl,wgr,icase,LGR_basis,LGR_scale,LGR_scale_factor,LGR_artificial_layer,lsponge);
...
%Klemp-Lily sponge
[Igamma,Igamma_volume,xmax_LGL,xt,xd]=create_sponge(coord,coord_cg,npoin,ngl_LGL,nelem_LGL,lsponge,sponge_method,sponge_shape,sponge_amp,sponge_exponent);
...
while time < time_final
   ...
   if (plot_movie == 1 && mod(itime,iplot) == 0)
    ...
    %Store Max values
   if (plot_qmax == 1 && mod(itime,iplot) == 0)
       ivector=ivector+1;
