...
%---------------------------------------------------------------------%
function rhs = apply_dss(rhs,intma,Mmatrix_h,Mmatrix_u,npoin,nelem,ngl,iperiodic)
...
rhs=0;
rhs_continuous(:,1)=Mmatrix_h\rhs_continuous(:,1);
rhs_continuous(:,2)=Mmatrix_u\rhs_continuous(:,2); 
for e=1:nelem
    for i=1:ngl
        ip=iperiodic(intma(i,e));
        for k=1:2
