...
%
%In addition, it uses Flux Differencing whereby the flux is the original (OR) flux as well as the averaged or 
%Kennedy-Gruber flux (KG). Both methods yield the same results
%
...
flux_method='KG'; %OR=original and KG=Kennedy-Gruber (or averaged flux)
...
numerical_flux_method='rusanov'; %rusanov or centered
...
noq=nop;
nq=noq + 1;
main_text=[space_method ':',flux_method];
...
for itime=1:ntime
    ...
    for s=1:stages
        ...
        mapL,mapR,npoin,nelem,nface,ngl,space_method,numerical_flux_method,flux_method);
        %------------Students Add Your Routines Here---------------%
        %------------Students Add Your Routines Here---------------%
