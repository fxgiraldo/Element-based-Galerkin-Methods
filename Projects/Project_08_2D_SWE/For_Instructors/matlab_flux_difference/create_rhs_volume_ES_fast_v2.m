...
%----------------------------------------------------------------------%
function rhs = create_rhs_volume_ES_fast_v2(q,u,v,ksi_x,ksi_y,eta_x,eta_y,jac,...
		 wnq,psi,dpsi,intma,iperiodic,npoin,nelem,ngl,flux_method)

%global arrays
rhs=zeros(npoin,1);
flux_es_ijkj=zeros(ngl,ngl,ngl,2);
flux_es_ijik=zeros(ngl,ngl,ngl,2);
node=zeros(ngl,ngl);
    ...
    for j=1:ngl
        for i=1:ngl
            ...
            for k=1:ngl
                ...
                flux_es_ijkj(i,j,k,1)=2*q_ave*u_ave;
                flux_es_ijkj(i,j,k,2)=2*q_ave*v_ave;
            end
        end
    end

    %Compute Flux Matrix: i,j,i,k
    for j=1:ngl
        for i=1:ngl
            I=node(i,j);
            q_I=q(I); u_I=u(I); v_I=v(I);
            for k=1:ngl
                ...
                flux_es_ijik(i,j,k,1)=2*q_ave*u_ave;
                flux_es_ijik(i,j,k,2)=2*q_ave*v_ave;
            end
        end
    end
    ...
    for j=1:ngl
        for i=1:ngl
            ...
            for k=1:ngl
                ...
                f_e=flux_es_ijkj(i,j,k,1);
                g_e=flux_es_ijkj(i,j,k,2);
                flux_e=h_e*( f_e*e_x + g_e*e_y);
                %Eta Terms
                h_n=psi(i,i)*dpsi(k,j);
                f_n=flux_es_ijik(i,j,k,1);
                g_n=flux_es_ijik(i,j,k,2);
