...
%global arrays
rhs=zeros(npoin,1);
flux_es_ijkj=zeros(ngl,ngl,ngl,ngl,2);
flux_es_ijik=zeros(ngl,ngl,ngl,ngl,2);
node=zeros(ngl,ngl);
    ...
    for j=1:ngl
        for i=1:ngl
            ...
            for k=1:ngl
                ...
                flux_es_ijkj(i,j,k,j,1)=2*q_ave*u_ave;
                flux_es_ijkj(i,j,k,j,2)=2*q_ave*v_ave;
            end
        end
    end

    %Compute Flux Matrix: IJIK
    for j=1:ngl
        for i=1:ngl
            I=node(i,j);
            q_I=q(I); u_I=u(I); v_I=v(I);
            for k=1:ngl
                ...
                flux_es_ijik(i,j,i,k,1)=2*q_ave*u_ave;
                flux_es_ijik(i,j,i,k,2)=2*q_ave*v_ave;
            end
        end
    end
    ...
    for j=1:ngl
        for i=1:ngl
            ...
            for k=1:ngl
                ...
                f_e=flux_es_ijkj(i,j,k,j,1);
                g_e=flux_es_ijkj(i,j,k,j,2);
                flux_e=h_e*( f_e*e_x + g_e*e_y);
                %Eta Terms
                h_n=psi(i,i)*dpsi(k,j);
                f_n=flux_es_ijik(i,j,i,k,1);
                g_n=flux_es_ijik(i,j,i,k,2);
