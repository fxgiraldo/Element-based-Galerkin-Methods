%---------------------------------------------------------------------%
%This file contains the solution for Project 8: 2D Shallow Water Equations using the unified CG/DG method with AGGP storage 
...
%
%Written by F.X. Giraldo on 11/2022
...
eq_set='shallow'; %=shallow or advection
...
plot_solution=1;
store_movie=0;
icase=7; %-------Case 1-6 are for Advection only
         %-------Case 1-6 are for Advection only
         %case number: 1 is a Gaussian in CW;
         %2 is a Gaussian along x;
         %3 is a Gaussian along y;
         %4 is a Gaussian along diagonal
         %5 is a Square in CW:
         %6 is a Square along x
         %-------Case 7 for Shallow only
         %-------Case 7 for Shallow only
         %7 is a Riemann problem for Shallow Water
%-------------------------Only Change These Lines------------------%
...
time=0;
qe=exact_solution(coord,npoin,time,icase,eq_set);
...
[Courant,vel,ds,dt] = compute_Courant(qe,intma,coord,nelem,ngl,Courant_max,eq_set);
...
while time < time_final
    ...
    [Courant,vel,ds,dt] = compute_Courant(qp,intma,coord,nelem,ngl,Courant_max,eq_set);
    ...
    for s=1:stages
        ...
        rhs = create_rhs(qp,ksi_x,ksi_y,eta_x,eta_y,jac,...
        ...
        %Hardwire Velocity for Advection
        if strcmp(eq_set,'advection') 
            for I=1:npoin
                qp(2,I)=(qe(2,I)/qe(1,I))*qp(1,I);
                qp(3,I)=(qe(3,I)/qe(1,I))*qp(1,I);
            end
        end
    end %s
    ...
    if (mod(itime,iplot) == 0 || itime == ntime)
        ...
        for i=1:npoin
            ...
            q_sol(ip_CG)=q_sol(ip_CG) + qp(1,i);
...
%Compute Norm
l2_norm=norm(qp(1,:)-qe(1,:))/norm(qe(1,:));
...
for i=1:npoin
    ip_CG=iperiodic_CG(DG_to_CG(i));
    q_sol(ip_CG)=q_sol(ip_CG) + qp(1,i);
    qe_sol(ip_CG)=qe_sol(ip_CG) + qe(1,i);
    lhowm(ip_CG)=lhowm(ip_CG)+1;
end %i
for i=1:npoin_CG
    q_sol(i)=q_sol(i)/lhowm(i);
...
if (store_movie == 1)
    vidObj=VideoWriter('SWE_movie.avi');
    open(vidObj);
    for i=1:iframe
        currFrame=M(i);
...
q_max=max(max(max(qp(1,:))));
q_min=min(min(min(qp(1,:))));
